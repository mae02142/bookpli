    ...
    actions: {
        initPlayer(token) {
            ...
            window.onSpotifyWebPlaybackSDKReady = () => {
                ...
                }).catch(error => console.error('Failed to connect the Web Playback SDK:', error));
            };
        ...
        togglePlay() {
            if (this.player) {
                this.player.togglePlay().catch(err => console.error('Error toggling play state:', err));
            }
        },
        ...
        setVolume(volume) {
            this.volume = volume;
            if (this.player) {
                this.player.setVolume(volume).catch(err => console.error('Error setting volume:', err));
            }
        },

        seekTrack(seconds) {
            if (this.player) {
                this.player.seek(seconds * 1000).catch(err => console.error('Error seeking track:', err));
        ...
        nextTrack() {
            if (this.player) {
                this.player.nextTrack().catch(err => console.error('Error skipping to next track:', err));
            }
        },

        previousTrack() {
            if (this.player) {
                this.player.previousTrack().catch(err => console.error('Error skipping to previous track:', err));
            }
        },
        ...
        async playContent(uri) {
            ...
            try {
                ...
                console.log(`Playing playlist: ${uri}`);
                    ...
                    console.warn('Unsupported URI type. Only playlists and tracks are supported.');
                }
            } catch (error) {
                console.error("Error playing content:", error.response ? error.response.data : error.message);
            }
        },
        ...
        this.currentTrack = {
            ...
            albumCover: current_track.album.images[0]?.url || "https://via.placeholder.com/200x200.png?text=No+Cover",
        ...
        this.updateInterval = setInterval(() => {
            if (this.isPlaying && this.currentTrack) {
                this.currentTime = Math.min(this.currentTime + 1, this.currentTrack.duration);
            }
        }, 1000);
        },
